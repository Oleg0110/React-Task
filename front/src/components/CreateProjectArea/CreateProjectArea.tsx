...
const CreateProjectArea: React.FC = () => {
  ...
  const onSubmit = (data: IOnSubmitProps) => ProjectsStore.pushProject(data.title, data.content)
    ...
    <div className={`${styles.createArea} ${styles[`createArea${responsive}`]}`}>
      ...
      <form className={styles.descriptionProjectBlock} onSubmit={handleSubmit(onSubmit)}>
        ...
        <textarea
          ...
          placeholder={t("createProject.descriptionPlaceholder")} className={`${styles.inputText} 
               ${styles[`inputText${responsive}`]}`} />
        {errors.content?.message && <p className={styles.errorContentPosition}>
          {errors.content?.message}
        </p>}
        <Button buttonStyle="fourthButtonStyle">{t("createProject.create")}</Button>
      </form>
    </div>
  )
}
